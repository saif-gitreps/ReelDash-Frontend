...
export default function Video({ id, url, username, description, likes, comments: initialComments }: VideoProps) {
  ...
  return (
    <div className="relative w-full h-screen bg-background">
      ...
      <div className="absolute right-4 bottom-20 flex flex-col items-center space-y-4">
        <Button variant="ghost" size="icon" onClick={handleLike} className="yellow-accent">
          <Heart className={`h-6 w-6 ${isLiked ? 'fill-current' : ''}`} />
        </Button>
        <span className="text-foreground">{likeCount}</span>
        <Button variant="ghost" size="icon" onClick={() => setShowComments(true)} className="yellow-accent">
        ...
        <Button variant="ghost" size="icon" onClick={handleSave} className="yellow-accent">
      ...
      {showComments && (
        <div className="absolute bottom-0 left-0 right-0 bg-secondary p-4 h-2/3">
          <div className="flex justify-between items-center mb-4">
            ...
            <Button variant="ghost" size="icon" onClick={() => setShowComments(false)} className="yellow-accent">
              <X className="h-6 w-6" />
            </Button>
          </div>
          <ScrollArea className="h-[calc(100%-8rem)] mb-4">
            {comments.map(comment => (
              <div key={comment.id} className="flex justify-between items-start mb-2 p-2 bg-muted rounded-md">
                <div>
          ...
          <div className="flex mt-4">
            ...
            />
            <Button onClick={handleAddComment} className="ml-2 yellow-accent-bg">Post</Button>
          </div>
        </div>
      )}
    </div>
